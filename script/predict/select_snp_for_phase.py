...
def FiterHeteSnpPhasing(ctgName, args):
    ...
    with opener(vcf_fn, 'rt') as f:
        for row in f:
            row = row.rstrip()
            if row[0] == '#': header.append(row + '\n'); continue
            columns = row.strip().split()
            ctg_name = columns[0]
            if ctgName and ctgName != ctg_name: continue
            ref_base = columns[3]; alt_base = columns[4]
            genotype = columns[9].split(':')[0].replace('|', '/')
            if len(ref_base) == 1 and len(alt_base) == 1:
                if genotype == '0/1' or genotype == '1/0': variant_dict[int(columns[1])] = row
    ...
    with open(out_path, 'w') as f:
        f.write(''.join(header))
        for key, row in sorted(variant_dict.items(), key=lambda x: x[0]): f.write(row + '\n')


def main():
    parser = ArgumentParser(description="Select heterozygous SNP candidates for WhatsHap phasing")
    parser.add_argument('--split_folder', type=str, default=None, help="Path to directory for split VCF files")
    parser.add_argument('--vcf_fn', type=str, default=None, help="Path of the input VCF file")
