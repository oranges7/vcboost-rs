...
while getopts ":o:b:v:m:r:t:c:d:ph" opt; do
  case $opt in
    o|out_path) out_path="$OPTARG" ;;
    b|bam_file) BAM_FILE_PATH="$OPTARG" ;;
    v|vcf) aim_vcf="$OPTARG" ;;
    m|model) model="$OPTARG" ;;
    r|ref_path) REFERENCE_FILE_PATH="$OPTARG" ;;
    t|threads?) THREADS="$OPTARG" ;;
    c|contig?) contig="$OPTARG" ;;
    d|model_path) model_path="$OPTARG" ;;
    p|phase?) phase=2 ;;
    h|help) echo "$help_message"; exit 0 ;;
    \?) echo "Invalid option: -$OPTARG" >&2; echo "$help_message"; exit 1 ;;
    :) echo "Option -$OPTARG requires an argument." >&2; echo "$help_message"; exit 1 ;;
    *) echo "Invalid option: -$OPTARG" >&2; echo "$help_message"; exit 1 ;;
  esac
done

if [ $# -eq 0 ]; then echo "No options provided. Use -h or -help for usage information." >&2; exit 1; fi
if [[ $# -lt 5 ]]; then echo "Insufficient parameters" >&2; echo $help_message; exit 1; fi
...
mkdir -p ${PHASE_VCF_PATH} ${PHASE_BAM_PATH} ${LOG_PATH} ${OUTPUT} ${out_path} ${OUTPUT_DATA}
